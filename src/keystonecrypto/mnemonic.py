"""BIP-39 English wordlist (2048 words, RFC order).
...
assert len(_ENGLISH_WORDLIST) == 2048
...
class Mnemonic:
    ...
    def __init__(self, phrase: str) -> None:
        # Internal constructor; use class methods.
        object.__setattr__(self, "_phrase", phrase)

    @classmethod
    def generate(cls, strength: int = 128) -> "Mnemonic":
    ...
    @classmethod
    def from_phrase(cls, phrase: str) -> "Mnemonic":
    ...
    @classmethod
    def from_entropy(cls, entropy) -> "Mnemonic":
        import hashlib
        from keystonecrypto.secret_bytes import SecretBytes
        if not isinstance(entropy, SecretBytes):
            raise TypeError("entropy must be a SecretBytes")
        ent = bytes(entropy)
    ...
    @property
    def entropy(self):
        """Reconstruct and return the original entropy as SecretBytes."""
        import hashlib
        from keystonecrypto.secret_bytes import SecretBytes
    ...
    def __repr__(self) -> str:
        return f"<Mnemonic words={len(self._phrase.split())} redacted>"

    def __eq__(self, other) -> bool:
        if not isinstance(other, Mnemonic):
            return NotImplemented
        return self._phrase == other._phrase

    def __hash__(self) -> int:
        return hash(self._phrase)
