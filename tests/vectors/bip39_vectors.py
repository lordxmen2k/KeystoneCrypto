...
IMPORTANT: per the BIP-39 specification, "The passphrase 'TREZOR' is used
for all vectors." Each trezor vector's `seed` is derived with passphrase
"TREZOR" (not an empty string). Our `Bip39Vector.passphrase` field makes
this explicit.

We ship 10 representative vectors covering 12/18/24-word mnemonics
(128/192/256-bit entropy). The Trezor test set does not include 15-word
(160-bit) or 21-word (224-bit) vectors — those bit lengths are valid
BIP-39 but not publicly verified, so we test them only via roundtrip.
"""
...
class Bip39Vector:
    ...
    mnemonic: str
    passphrase: str
    seed_hex: str  # 64-byte PBKDF2 seed, derived with `passphrase`
